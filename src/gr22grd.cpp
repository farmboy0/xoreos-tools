...
template<typename It>
	uint32_t
	crc32(It first, It last, uint32_t initial = 0xffffffff) {
		static auto const table = generate_crc32_table();

		return ~std::accumulate(first, last, initial,
		    [](uint32_t checksum, uint8_t value) {return table[(checksum ^ value) & 0xFFu] ^ (checksum >> 8);});
	}

void
decompress(std::string infile, std::string outfile) {
	...
	std::vector<
	    std::tuple<decltype(section), std::vector<decltype(relocation)>, std::vector<decltype(marshalling)>,
	...
	for (size_t i = 0; i < info.sections_count; ++i) {
		...
		sections.emplace_back(
		    std::make_tuple(section, std::move(relocations), std::move(marshallings), std::move(data)));
	}
	...
	for (auto& s : sections) {
		...
		info.crc32 = gr2::crc32(reinterpret_cast<char*>(relocations.data()),
		    reinterpret_cast<char*>(relocations.data() + relocations.size()), info.crc32);
		info.crc32 = gr2::crc32(reinterpret_cast<char*>(marshallings.data()),
		    reinterpret_cast<char*>(marshallings.data() + marshallings.size()), info.crc32);
	...
	for (auto& s : sections) {
		...
		for (auto& r : relocations) {
			*reinterpret_cast<uint32_t*>(&data[r.offset]) = std::get < 0
			    > (sections[r.target_section]).data_offset + r.target_offset;
		}
	}
