...
struct decoder {
	...
	decode(uint16_t max);
	uint16_t
	commit(uint16_t max, uint16_t val, uint16_t err);
	uint16_t
	decode_and_commit(uint16_t max);
};
...
struct dictionary {
	...
	uint32_t
	decompress_block(decoder& dec, uint8_t* dbuf);
};
...
uint16_t
decoder::commit(uint16_t max, uint16_t val, uint16_t err) {
...
uint16_t
decoder::decode_and_commit(uint16_t max) {
...
void
weighwindow::rebuild_weights() {
	std::transform(std::begin(this->weights), std::end(this->weights), std::begin(this->weights),
...
std::pair<uint16_t*, uint16_t>
weighwindow::try_decode(decoder &dec) {
	...
	if ((this->weights.size() >= this->ranges.size()) && (dec.decode_and_commit(2) == 1)) {
...
dictionary::dictionary(parameters& params) :
	decoded_size(0), backref_size(0),

	decoded_value_max(params.decoded_value_max), backref_value_max(params.backref_value_max),
	    lowbit_value_max(std::min(backref_value_max + 1, 4u)),
	    midbit_value_max(std::min(backref_value_max / 4 + 1, 256u)), highbit_value_max(backref_value_max / 1024u + 1),
...
uint32_t
dictionary::decompress_block(decoder& dec, uint8_t* dbuf) {
...
void
decompress(uint32_t csize, uint8_t* cbuf, uint32_t step1, uint32_t step2, uint32_t dsize, uint8_t* dbuf) {
	if (csize == 0)
		return;
	...
	for (uint32_t i = 0; i < 3; ++i) {
		auto dic = make_unique<dictionary>(params[i]);
